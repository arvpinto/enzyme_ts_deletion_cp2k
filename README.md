...
```js
paste <(for i in RES_*; do echo "$i" | sed 's/RES_//g'; done) <(for i in RES_*; do grep "Total FORCE" "$i"/res_qmmm_TS.out | tail -n -1 ; done | awk '{print $9}') <(for i in RES_*; do grep "Total FORCE" "$i"/res_qmmm_R.out | tail -n -1 ; done | awk '{print $9}') | awk '{print $1,($2-$3)*627.509}' | sort -n -k1,1 > energy_differences.dat
```
